...
export class QuestionPage implements OnInit {
  ...
  nextPlace(){
    ...
    if (this.counter==0){
      arrayAnswers[0]=JSON.stringify(dato);
      this.storage.set('answersTeam', JSON.stringify(arrayAnswers));
    }else{
      this.storage.get('answersTeam').then((val) => {
        arrayAnswers=JSON.parse(val);
        arrayAnswers[this.counter]=JSON.stringify(dato);
      });
      this.storage.set('answersTeam', arrayAnswers);
    }
    this.counter++;
  ...
  finishTheGame(){
    this.storage.get('answersTeam').then((val) => {
      this.datosFinales=JSON.parse(val);
    });

  }

  printValue(){
    this.presentToast(this.answerTeamOfTheQuestion)
  }
